...
def load_metadata(metadata_file_name="_coursera"):
    ...
    try:
        with open(metadata_file_name, "r") as metadata_file:
            ...
            if len(name) <= 0:
                print(
                    "Empty assignment name in _coursera file: %s" % metadata_file_name
                )
                quit()
...
def compute(metadata, solver_file_override=None):
    ...
    for problem in selected_problems:
        ...
        if not os.path.isfile(solver_file):
            print(
                'Unable to locate assignment file "%s" in the current working directory.'
                % solver_file
            )
            continue
...
def output(input_file, solver_file):
    ...
    if not (isinstance(solution, str) or isinstance(solution, unicode)):
        print(
            "Warning: the solver did not return a string.  The given object will be converted with the str() method."
        )
...
def submit_solution(assignment_key, email_address, token, results):
    ...
    try:
        ...
        if (
            "details" in response
            and response["details"] != None
            and "learnerMessage" in response["details"]
        ):
            return e.code, response["details"]["learnerMessage"]
        else:
            return (
                e.code,
                "Unexpected response code, please contact the "
    ...
    if code >= 200 and code <= 299:
        return code, "Your submission has been accepted and will be " "graded shortly."
...
def build_parser():
    ...
    parser.add_argument(
        "-m", "--metadata", help="overrides the '_coursera' metadata file"
    )

    parser.add_argument(
        "-c", "--credentials", help="overrides the '_credentials' credentials file"
    )

    parser.add_argument(
        "-rs",
        "--record_submission",
        help="records the submission(s) as files",
        action="store_true",
    )
